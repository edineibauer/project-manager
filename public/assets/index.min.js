...
+function ($) {
    ...
    CarouselSwipe.prototype.swipe = function (percent) {
        ...
        if (percent < 0) {
            ...
            this.$next.carousel_transition(!1).addClass('next').carousel_offset(percent + 100)
        } else {
            this.$next.carousel_offset(!1).removeClass('next').carousel_transition(!0)
            if (!this.$prev.length || this.$prev.hasClass('active')) return
            this.$prev.carousel_transition(!1).addClass('prev').carousel_offset(percent - 100)
        }
        $active.carousel_transition(!1).carousel_offset(percent)
    }
...
(function ($) {
    jQuery.vimeo_player = {
        ...
        controls: {play: "P", pause: "p", mute: "M", unmute: "A", fullscreen: "O", showSite: "R", logo: "V"},
        buildPlayer: function (options) {
            var isIframe = function () {
                var isIfr = !1;
                try {
                    if (self.location.href != top.location.href) isIfr = !0
                } catch (e) {
                    isIfr = !0
                }
                return isIfr
            ...
            return this.each(function () {
                ...
                if (!vimeo_player.canGoFullScreen) vimeo_player.opt.realfullscreen = !1;
                vimeo_player.isAlone = !1;
                vimeo_player.hasFocus = !0;
                vimeo_player.videoID = this.opt.videoURL ? get_vimeo_videoID(this.opt.videoURL) : $vimeo_player.attr("href") ? get_vimeo_videoID($vimeo_player.attr("href")) : !1;
                ...
                vimeo_player.playerBox.css({
                    ...
                    overflow: "hidden",
                    left: 0
                }).attr({src: "https://player.vimeo.com/video/" + vimeo_player.videoID + "?background=1&autopause=0"});
                if (!jQuery.browser.mobile || vimeo_player.canPlayOnMobile)
                    wrapper.append(vimeo_player.playerBox); else {
                ...
                vimeo_player.playerBox.css({opacity: 1});
        ...
        setVolume: function (val) {
            ...
            if (!val && !vimeo_player.opt.vol && vimeo_player.isMute)
                jQuery(vimeo_player).v_unmute(); else if ((!val && !vimeo_player.isMute) || (val && vimeo_player.opt.vol == val)) {
                if (!vimeo_player.isMute)
                    jQuery(vimeo_player).v_unmute(); else jQuery(vimeo_player).v_mute()
            } else {
        ...
        buildControls: function (vimeo_player) {
            ...
            var playpause = jQuery("<span>" + jQuery.vimeo_player.controls.play + "</span>").addClass("vimeo_player_pause vimeo_icon").click(function () {
                if (vimeo_player.state == 1) jQuery(vimeo_player).v_pause(); else jQuery(vimeo_player).v_play()
            });
            ...
            var volumeBar = jQuery("<div/>").addClass("vimeo_player_volume_bar").css({display: "inline-block"});
            ...
            var progressBar = jQuery("<div/>").addClass("vimeo_player_pogress").css("position", "absolute").click(function (e) {
                ...
                vimeo_player.controlBar.find(".vimeo_player_loaded").css({width: 0});
            ...
            volumeBar.simpleSlider({
                initialval: vimeo_player.opt.vol, scale: 100, orientation: "h", callback: function (el) {
                    if (el.value == 0) {
                        jQuery(vimeo_player).v_mute()
                    } else {
                        jQuery(vimeo_player).v_unmute()
        ...
        fullscreen: function (real) {
            ...
            if (!vimeo_player.isAlone) {
                ...
                } else videoWrapper.css({zIndex: 10000}).fadeTo(1000, 1);
                ...
                if (real) {
                    ...
                    videoWrapper.fadeTo(1000, vimeo_player.opt.opacity).css({zIndex: 0})
                }
                fullScreenBtn.html(jQuery.vimeo_player.controls.fullscreen);
                vimeo_player.isAlone = !1
            }
        ...
        init: function (c) {
            return this.each(function () {
                ...
                a.opt.readonly || (d.on(b.simpleSlider.events.start, function (c) {
                    ...
                    b.browser.mobile && (c = c.changedTouches[0]);
                    a.canSlide && (b(document).css({cursor: "default"}), d.updateSliderVal(c), c.preventDefault(), c.stopPropagation())
...
function initClientCarousel(card) {
    var $target = $(card), countElems = $target.find('.carousel-item').length,
...
;(function ($) {
    ...
    $.fn.footerReveal = function () {
        ...
        function initReveal() {
            if ($this.outerHeight() <= $win.outerHeight()) {
                $this.css({'z-index': -999, position: 'fixed', bottom: 0});
                $this.css({'width': $prev.outerWidth()});
                $prev.css({'margin-bottom': $this.outerHeight()})
            } else {
                $this.css({'z-index': '', position: '', bottom: ''});
                $this.css({'width': ''});
    ...
    $(function () {
        ...
        function initParallax(card) {
            setTimeout(function () {
                $(card).outerFind('.mbr-parallax-background').jarallax({speed: 0.6}).css('position', 'relative')
            }, 0)
        }
                    ...
                    if (paramName === 'bg') {
                        ...
                        switch (key) {
                            case 'type':
                                ...
                                break;
                            case 'value':
                                if (value.type === 'image' && value.parallax === !0) {
                                    initParallax(event.target)
                                }
                                break;
                            case 'parallax':
        ...
        $(document).on('add.cards delete.cards', function (event) {
            ...
            fixedTopTimeout = setTimeout(function () {
                ...
                $('.mbr-fixed-top:first').each(function () {
                    fixedTop = {breakPoint: $(this).offset().top + $(this).height() * 3, fixed: !1, elm: this};
                    $(window).scroll()
                })
            }, 650)
        });
            ...
            $(card).outerFind('[data-bg-video]').each(function () {
                ...
                if (parsedUrl && (/youtube/g.test(parsedUrl[3]) || /vimeo/g.test(parsedUrl[3]))) {
                    if (parsedUrl && /youtube/g.test(parsedUrl[3])) {
                        ...
                        $('<img>').on('load', function () {
                            if (120 === (this.naturalWidth || this.width)) {
                                ...
                                switch (file) {
                                    ...
                                    default:
                                        if (isBuilder) {
                                            $img.css('background-image', 'url("public/assets/images/no-video.jpg")').show()
                                        }
                                }
                            } else {
                        ...
                        request.open('GET', 'https://vimeo.com/api/v2/video/' + parsedUrl[6] + '.json', !0);
                    ...
                    $img.css('background-image', 'url("public/assets/images/video-placeholder.jpg")').show()
                }
            })
        }
        ...
        $('.cols-same-height .mbr-figure').each(function () {
            ...
            function setNewSize() {
                $img.css({width: '', maxWidth: '', marginLeft: ''});
                if (imgH && imgW) {
    ...
    if (!isBuilder) {
        $('.mbr-arrow').on('click', function (e) {
            ...
            $('html, body').stop().animate({scrollTop: offset.top}, 800, 'linear')
        })
    }
    ...
    if (!isBuilder) {
        if (!$.isMobile()) {
            var $menu = $('section.menu'), $width = $(window).width(),
    ...
    if (!isBuilder) {
        ...
        if ($('.mbr-slider.carousel').length != 0) {
            $('.mbr-slider.carousel').each(function () {
                var $slider = $(this), controls = $slider.find('.carousel-control'),
                ...
                $slider.on('slide.bs.carousel', function () {
                    ...
                    $slider.carousel({keyboard: !1})
                }).on('slid.bs.carousel', function () {
                    controls.unbind('click');
                    indicators.unbind('click');
                    $slider.carousel({keyboard: !0});
...
(function ($) {
    ...
    if (!isBuilder) {
        ...
        window.onYouTubeIframeAPIReady = function () {
            ...
            $('.video-slide').each(function (i) {
                $('.video-container').eq(i).append('<div id ="mbr-video-' + i + '" class="mbr-background-video" data-video-num="' + i + '"></div>').append('<div class="item-overlay"></div>');
                $(this).attr('data-video-num', i);
                if ($(this).attr('data-video-url').indexOf('vimeo.com') !== -1) {
                    var options = {id: $(this).attr('data-video-url'), width: '100%', height: '100%', loop: !0};
    ...
    function updateMasonry(event) {
        ...
        var $section = $(event.target), allItem = $section.find('.mbr-gallery-filter-all');
        var filterList = [];
    ...
    function fitLightbox() {
        ...
        $lightbox.each(function () {
            ...
            $modalDialog.css({width: parseInt(setWidth), top: setTop + windowPadding})
        })
    }
...
jQuery(function ($) {
        ...
        Form.prototype.send = function (data, beforeStart) {
            ...
            $ajax(API_URL, {
                ...
                data: JSON.stringify({
                    ...
                    form: {title: this.__title__, data: (arguments.length ? data : this.__data__)}
                })
    ...
    var isValid = function (input) {
        ...
        var isValid = !0, value = $(input).val(), type = $(input).attr('type');
        if (value) isValid = !('email' === type && !/^([^@]+?)@(([a-z0-9]-*)*[a-z0-9]+\.)+([a-z0-9]+)$/i.test(value)); else if ($(input).attr('required')) isValid = !1;
        $(input)[(isValid ? 'remove' : 'add') + 'Class']('form-invalid');
        return isValid
    };
    $('[data-form-type="formoid"]').each(function () {
        var form, $this = $(this), $form = $this.is('form') ? $this : $this.find('form'),
        ...
        $form.submit(function (event) {
            ...
            $this.find('[data-form-field]').each(function () {
                if (!isValid(this))
                    isValidForm = !1;
                data.push([$(this).attr('data-form-field') || $(this).attr('name'), $(this).val()])
            });
            ...
            form.send(data).done(function (message) {
                ...
                $submit.removeClass('btn-loading').prop('disabled', !1)
            })
        })
    })
});
...
$(".btn-form").off("click").on("click", function() {
    post('project-manager', 'contato', {mensagem: $("#mensagem").val(), nome: $("#nome").val(), telefone: $("#telefone").val(), email: $("#email").val()}, function(g) {
        if(typeof g === "number") {
            toast("mensagem enviada!", 3000, "toast-success");
            $('.mbr-form')[0].reset();
        } else {
            toast("Erro no Formulário!", 3000, "toast-warning");
        }
    });
});